...
{
	ISeqInStream vt;
	sc::BinaryStream* inStream;
};

struct CSeqOutStreamWrap
{
	ISeqOutStream vt;
	sc::BinaryStream* outStream;
};
...
namespace sc
{
	CompressionError LZMA::decompressStream(CLzmaDec* state, SizeT unpackSize, BinaryStream& inStream, BinaryStream& outStream)
	{
		...
		LzmaDec_Init(state);
		for (;;) // FIXME: Why ;; instead of while(true)? Okay, anyway it works
		{
			if (inPos == inSize)
			{
			...
			}
			{
				...
				res = LzmaDec_DecodeToBuf(state, outBuf + outPos, &outProcessed,
					inBuf + inPos, &inProcessed, finishMode, &status) == 0 ? CompressionError::OK : CompressionError::DATA_ERROR;
				inPos += inProcessed;
				outPos += outProcessed;
	...
	CompressionError LZMA::decompress(BinaryStream& inStream, BinaryStream& outStream)
	{
		CLzmaDec state;
	...
	CompressionError LZMA::compress(BinaryStream& inStream, BinaryStream& outStream)
	{
		CLzmaEncHandle enc;
		SRes res;
		CLzmaEncProps props;
