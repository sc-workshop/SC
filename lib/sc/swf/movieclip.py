...
class MovieClipModifier(Writable):
    ...
    def load(self, swf, tag: int):
        ...
        swf.modifers_ids.append(self.id)
    ...
    def __eq__(self, instance):
        if isinstance(instance, MovieClip):
            return self.frame_rate == instance.frame_rate \
                   ...
                   and (len(instance.frames) != self.frames and False not in [self.frames[i] == instance.frames[i] for i
                                                                              in range(len(self.frames))])
        return False
